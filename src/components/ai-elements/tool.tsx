...
const TOOL_META: Record<string, { label: string; icon: typeof File01Icon }> = {
  ...
  bash_list: { label: "后台任务", icon: TerminalIcon },
  ...
  todo_write: { label: "任务", icon: CheckListIcon },
};
...
function deriveSummary(toolName: string, input: unknown): string | null {
  ...
  switch (toolName) {
    ...
    case "bash_kill": {
      ...
      return typeof handle === "number" ? `#${handle}` : str("id");
    }
    case "grep":
...
const ToolImpl = ({
  ...
  return (
    ...
    >
      ...
      {hasDetails && (
        <CollapsibleContent
          className={cn("gencode-collapsible-content")}
        >
...
function ToolInput({ toolName, input }: { toolName: string; input: unknown }) {
  ...
  return (
    <div className="space-y-1">
      <div className="text-[13px] font-medium text-muted-foreground">Input</div>
      <CodeBlockMini
        code={
...
function renderInputPreview(
  ...
  if (toolName === "bash_run" || toolName === "bash_background") {
    ...
    return (
      <div className="space-y-1">
        ...
        <pre className="overflow-auto rounded bg-muted/40 p-2 font-mono text-[13px] leading-relaxed">
          {cmd}
        </pre>
      </div>
    );
  ...
  if (
    ...
    if (!path) return null;
    return (
      <div className="font-mono text-[13px] text-muted-foreground">{path}</div>
    );
  }
  if (toolName === "grep") {
...
function renderToolOutput(toolName: string, output: unknown): ReactNode | null {
  ...
  if (toolName === "list_directory") {
    ...
    if (entries.length === 0) {
      return (
        <div className="text-[13px] italic text-muted-foreground">empty</div>
      );
    }
    ...
    return (
      <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 font-mono text-[13px]">
        {dirs.map((e) => (
          <div
            key={`d-${e.name}`}
  ...
  if (toolName === "grep") {
    ...
    return (
      <div className="space-y-1">
        <div className="max-h-72 overflow-auto rounded bg-muted/30 font-mono text-[13px]">
  ...
  if (toolName === "glob") {
    ...
    return (
      <div className="max-h-60 overflow-auto rounded bg-muted/30 px-2 py-1 font-mono text-[13px]">
  ...
  if (toolName === "bash_background") {
    const handle = typeof o.handle === "number" ? String(o.handle) : null;
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center gap-1.5">
        ...
        {timedOut ? (
          <span className="rounded bg-amber-500/15 px-1.5 py-0.5 font-mono text-[13px] text-amber-700 dark:text-amber-400">
            timed out
          </span>
        ) : null}
        {truncated ? (
      ...
      <pre className="max-h-72 overflow-auto rounded bg-muted/40 p-2 font-mono text-[13px] leading-relaxed whitespace-pre-wrap">
...
function CodeBlockMini({ code }: { code: string; language: string }) {
  ...
  return (
    <pre className="max-h-60 overflow-auto rounded bg-muted/40 p-2 font-mono text-[13px] leading-relaxed text-foreground whitespace-pre-wrap">
      {code}
    </pre>
  );
}
...
function SuggestCommandCard({
  ...
  return (
    <div className="space-y-1.5">
      {explanation ? (
        <div className="text-[13px] text-muted-foreground">{explanation}</div>
      ) : null}
      <div className="flex items-stretch gap-1.5 rounded bg-muted/40 overflow-hidden">
        <pre className="flex-1 overflow-auto p-2 font-mono text-[13px] leading-relaxed">
          {command}
        </pre>
        <button
          type="button"
