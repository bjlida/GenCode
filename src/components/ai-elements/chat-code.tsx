...
function GeneratingPlaceholder({ label }: { label: string }) {
  return (
    <div className="not-prose my-2 flex items-center gap-2 rounded-lg border border-border/50 bg-muted/30 px-3 py-2 text-[13px] text-muted-foreground">
  ...
  return (
    <div className="not-prose my-2 overflow-hidden rounded-lg border border-border/50 bg-muted/40">
      ...
      <div className="border-t border-border/40 bg-background/40">
        <pre
          className={cn(
            "m-0 overflow-x-auto px-3 py-2 font-mono text-[13px] leading-relaxed text-foreground",
...
function RunInTerminalButton({ command }: { command: string }) {
  ...
  return (
    <Button
      ...
      variant="ghost"
      onClick={onRun}
      className="h-5 gap-1 px-1.5 text-[13px] font-medium text-muted-foreground hover:text-foreground"
      aria-label="在活动终端中运行"
      title="在活动终端中运行"
    >
      <HugeiconsIcon
