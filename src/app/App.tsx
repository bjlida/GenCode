...
const SIDEBAR_WIDTH_STORAGE_KEY = "gencode.sidebar.width";
const SIDEBAR_VIEW_STORAGE_KEY = "gencode.sidebar.view";
...
function readSidebarWidth(): number {
  try {
    ...
    if (stored === null) return 0; // First launch — start collapsed
...
export default function App() {
  ...
  const toggleSidebar = useCallback(() => {
    ...
    if (p.getSize().asPercentage <= 0) {
      const restore = sidebarWidthRef.current > 0
    ...
    (view: SidebarViewId) => {
      ...
      if (collapsed) {
        const restore = sidebarWidthRef.current > 0
          ? sidebarWidthRef.current
          : SIDEBAR_DEFAULT_WIDTH;
        if (panel) panel.resize(`${restore}px`);
  ...
  const switchWorkspace = useCallback(
    async (env: WorkspaceEnv) => {
      ...
      if (dirty) {
        window.alert("请先保存或关闭未保存的编辑器标签页，再切换工作区。");
        return;
      }
      ...
      (event) => {
        ...
        void (async () => {
          try {
            ...
            if (!parsed.ok) {
              console.warn("[gencode] theme not applied:", parsed.error);
              return;
            }
            await saveCustomTheme(parsed.theme);
          } catch (e) {
            console.warn("[gencode] theme ingest failed:", e);
          }
        })();
      },
    );
  ...
  const handleAttachFileToAgent = useCallback(
    (path: string) => {
      ...
      window.dispatchEvent(
        new CustomEvent<string>("gencode:ai-attach-file", { detail: path }),
      );
      openPanel();
      focusInput(null);
    },
    ...
    const onUp = (e: MouseEvent) => {
      ...
      setTimeout(() => {
        ...
        if (text && text.trim().length > 0) {
          setAskPopup(getAskPopupPoint({ x: e.clientX, y: e.clientY }));
        } else {
          setAskPopup(null);
        }
      }, 0);
  ...
  }, [captureActiveSelection, getAskPopupPoint]);
                  ...
                  <div className="min-h-0 flex-1">
                    {sidebarView === "explorer" ? (
                      <FileExplorer
                        ...
                        onChangeRoot={handleChangeExplorerRoot}
                      />
                    ) : (
                      <SourceControlPanel
                        open
                ...
                <AlertDialogDescription>
                  {pendingDeleteTabs?.length === 1
                    ? (() => {
                        ...
                        return title
                          ? `"${title}" 有未保存的更改。文件已被删除。确定关闭吗？`
                          : "此文件有未保存的更改。文件已被删除。确定关闭吗？";
                      })()
                    : `${pendingDeleteTabs?.length ?? 0} 个文件有未保存的更改，且已被删除。确定全部关闭吗？`}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
