      ...
      <aside className="flex h-full min-w-0 flex-col bg-card/80 backdrop-blur [contain:layout_style]">
        <header className="flex shrink-0 items-center justify-between gap-2 border-b border-border/50 px-3 pb-2.5 pt-3">
          <div className="flex min-w-0 items-center gap-1.5">
            <div className="inline-flex min-w-0 items-center gap-1.5 rounded-md bg-foreground/5 px-2 py-1 text-[13px] font-medium leading-none text-foreground transition-colors hover:bg-foreground/10">
              <HugeiconsIcon
            ...
            {scm.status && (scm.status.ahead > 0 || scm.status.behind > 0) ? (
              <div className="flex shrink-0 items-center gap-0.5 text-[11.5px] font-semibold tabular-nums leading-none text-muted-foreground">
                {scm.status.ahead > 0 ? (
            ...
            {scm.status?.isDetached ? (
              <span className="rounded bg-muted/55 px-1.5 py-0.5 text-[11.5px] font-medium uppercase tracking-wider text-muted-foreground">
                detached
              </span>
            ) : null}
          </div>
                  ...
                  : isDiverged
                    ...
                    : !hasUpstream
                      ...
                      : (scm.status?.behind ?? 0) === 0
                        ? "已是最新"
                        : `拉取 ${scm.status?.behind ?? 0} 个提交（快进合并）`
              }
              disabled={!canPull}
              onClick={handlePull}
              side="bottom"
        ...
        {onOpenGitGraph ? (
          ...
          >
            <HugeiconsIcon
              ...
              className="shrink-0"
            />
            <span className="flex-1 text-[13px] font-medium">提交图</span>
            <HugeiconsIcon
            ...
            <div className="relative shrink-0 space-y-2 border-b border-border/40 bg-gradient-to-b from-card/65 to-card/30 px-2.5 pb-2.5 pt-2.5">
              ...
              >
                <Textarea
                  ...
                  rows={3}
                  className={cn(
                    "min-h-[72px] border-  resize-none rounded-lg  bg-transparent px-3 pb-7 pt-2.5 text-[11.5px] leading-snug shadow-none placeholder:text-muted-foreground/65 focus-visible:ring-0 focus:border-0",
                  )}
                />
                <div className="pointer-events-none absolute inset-x-3 bottom-1.5 flex items-center justify-between p-1 gap-2 text-[11.5px] tabular-nums text-muted-foreground/55">
                  {scm.commitMessage.length > 0 ? (
              ...
              <div className="grid w-full grid-cols-2 gap-1.5">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      size="xs"
                      className="h-7 cursor-pointer text-[13px] font-semibold tracking-tight shadow-sm disabled:cursor-not-allowed disabled:shadow-none"
                      disabled={!canCommit}
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      size="xs"
                      variant="secondary"
                      className="h-7 cursor-pointer text-[13px] font-medium disabled:cursor-not-allowed"
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {scm.pendingDiscard?.scope === "all"
                ? `这将放弃 ${scm.pendingDiscard.label}，此操作不可撤销。`
                : scm.pendingDiscard
                  ? `放弃 "${scm.pendingDiscard.label}" 中的更改？此操作不可撤销。`
                  : null}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-1.5 px-4 text-center">
      ...
      <div className="text-[11.5px] leading-snug text-muted-foreground">
        当前分支 <span className="font-mono text-foreground/80">{repoLabel}</span>
      </div>
    </div>
  );
}
...
function DivergedBanner() {
  return (
    <div className="mx-2 mt-1 flex h-7 items-center gap-1.5 rounded-md border border-border/60 bg-foreground/[0.04] px-2 text-[11.5px] leading-none text-muted-foreground">
      <HugeiconsIcon
        icon={Alert02Icon}
        size={11}
        strokeWidth={1.9}
...
function ListHeader({
  ...
  return (
    <div className="flex h-7 items-center gap-2 px-3">
      <span className="text-[11.5px] font-semibold uppercase tracking-[0.16em] text-muted-foreground/85">
        更改
      </span>
      <span className="inline-flex h-4 min-w-4 items-center justify-center rounded-full border border-border/60 px-1 text-[11.5px] font-semibold tabular-nums text-muted-foreground">
        {row.count}
      </span>
      <label className="ml-auto flex shrink-0 cursor-pointer select-none items-center gap-1.5 text-[11.5px] font-medium text-muted-foreground hover:text-foreground">
        <span>全选</span>
        <Checkbox
          aria-label="暂存所有更改"
    ...
    >
      ...
      >
        ...
        <div className="flex min-w-0 flex-1 items-baseline gap-1.5 leading-none">
          ...
          {pathLabel ? (
            <span className="min-w-0 flex-1 truncate text-[11.5px] leading-tight text-muted-foreground/75">
              {pathLabel}
            </span>
          ) : null}
        </div>
...
function IconActionButton({
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent
        side={side}
        className={cn(SOURCE_CONTROL_TOOLTIP_CLASS, "text-[13px]")}
      >
        {label}
      </TooltipContent>
    </Tooltip>
...
function CommitFeedback({
  ...
  return (
    <div
      className={cn(
        "pointer-events-none absolute inset-x-3 top-[calc(100%-0.25rem)] z-20 flex min-w-0 items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-[13px] leading-snug shadow-lg shadow-black/15 backdrop-blur transition-all duration-200",
