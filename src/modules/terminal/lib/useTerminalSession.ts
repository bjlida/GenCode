    ...
    return {
      ...
      kickPty: (cols, rows) => {
        ...
        pty
          ...
          .catch((e) => console.warn("[gencode] kickPty failed:", e));
      },
    };
  },
  evictLeaf(leafId) {
...
function ensureSession(leafId: number, initialCwd?: string): Session {
  ...
  session.ready = (async () => {
    try {
      ...
      console.warn("[gencode] font loading failed, proceeding without:", e);
      // Still resolve so attachSession fires — terminal must work even
      // if font preloading throws.
    }
  })();
...
export async function respawnSession(
  ...
  try {
    ...
    s.ptyOpening = false;
    console.error("[gencode] respawn openPty failed:", e);
    return;
  }
  s.ptyOpening = false;
  if (s.disposed) {
