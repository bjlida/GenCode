...
function getRecycler(): HTMLDivElement {
  ...
  el.setAttribute("data-gencode-recycler", "");
  el.style.cssText =
...
function bindSlot(slot: Slot, p: AcquireParams): void {
  ...
  if (p.snapshot) {
    try {
      ...
      console.warn("[gencode] snapshot replay failed:", e);
    }
  }
  if (p.altScreen) {
    p.drainRing(() => {});
...
function serializeSlot(slot: Slot): SerializeOutput {
  ...
  try {
    ...
    console.warn("[gencode] serialize failed:", e);
  }
  return {
    snapshot,
    cols: slot.term.cols,
...
function disposeSlotWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[gencode-webgl] dispose failed:", e);
  }
  try {
    const r = (
