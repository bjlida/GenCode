...
const GENCODE_MD_MAX_BYTES = 32 * 1024;
type MemoryCacheEntry = { content: string | null; mtime: number };
const projectMemoryCache = new Map<string, MemoryCacheEntry>();

async function readGencodeMd(workspaceRoot: string | null): Promise<string | null> {
  if (!workspaceRoot) return null;
  const path = `${workspaceRoot.replace(/\/$/, "")}/GENCODE.md`;
  ...
  try {
    ...
    const content =
      r.content.length > GENCODE_MD_MAX_BYTES
        ? r.content.slice(0, GENCODE_MD_MAX_BYTES)
        : r.content;
...
type Deps = {
  ...
  getQwenBaseURL?: () => string | undefined;
  getQwenModelId?: () => string | undefined;
  getZhipuBaseURL?: () => string | undefined;
  getZhipuModelId?: () => string | undefined;
  getMoonshotBaseURL?: () => string | undefined;
  getMoonshotModelId?: () => string | undefined;
  getAiTemperature?: () => number | undefined;
  getAiMaxTokens?: () => number | undefined;
...
export function createContextAwareTransport(deps: Deps) {
  const run = async (options: SendOptions) => {
    ...
    const projectMemory = await readGencodeMd(live.workspaceRoot);
    ...
    const result = await runAgentStream({
      ...
      qwenBaseURL: deps.getQwenBaseURL?.(),
      qwenModelId: deps.getQwenModelId?.(),
      zhipuBaseURL: deps.getZhipuBaseURL?.(),
      zhipuModelId: deps.getZhipuModelId?.(),
      moonshotBaseURL: deps.getMoonshotBaseURL?.(),
      moonshotModelId: deps.getMoonshotModelId?.(),
      temperature: deps.getAiTemperature?.(),
      maxTokens: deps.getAiMaxTokens?.(),
