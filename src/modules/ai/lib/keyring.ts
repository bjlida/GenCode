...
export async function getKey(provider: ProviderId): Promise<string | null> {
  ...
  const v = await invoke<string | null>("secrets_get", {
    ...
    // Keychain read failed (locked, corrupted, etc). Log so it's detectable
    // in devtools but return null so the caller still falls through gracefully.
    console.warn(`keyring: failed to read key for ${provider}`);
    return null;
  });
