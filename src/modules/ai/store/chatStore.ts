...
// Flush pending writes on page unload so the last ~300ms of messages
// are not lost when the window is closed.
...
function makeChat(sessionId: string): Chat<UIMessage> {
  ...
  const transport = createContextAwareTransport({
    ...
    getQwenBaseURL: () => usePreferencesStore.getState().qwenBaseURL,
    getQwenModelId: () => usePreferencesStore.getState().qwenModelId,
    getZhipuBaseURL: () => usePreferencesStore.getState().zhipuBaseURL,
    getZhipuModelId: () => usePreferencesStore.getState().zhipuModelId,
    getMoonshotBaseURL: () => usePreferencesStore.getState().moonshotBaseURL,
    getMoonshotModelId: () => usePreferencesStore.getState().moonshotModelId,
    getAiTemperature: () => usePreferencesStore.getState().aiTemperature,
    getAiMaxTokens: () => usePreferencesStore.getState().aiMaxTokens,
    onStep: (step) => {
