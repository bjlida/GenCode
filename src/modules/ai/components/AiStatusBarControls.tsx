...
function TabButton({
  ...
  return (
    ...
    >
      ...
      {label}
      {count != null ? (
        <span className="rounded-full bg-muted/60 px-1.5 text-[13px] tabular-nums text-muted-foreground">
          {count}
        </span>
      ) : null}
    </button>
...
function ProviderHeader({ providerId }: { providerId: ProviderId }) {
  ...
  if (!p) return null;
  return (
    <div className="flex items-center gap-1.5 px-3 pt-1 pb-1.5 text-[13px] font-medium tracking-tight text-muted-foreground/90">
      <HugeiconsIcon
        icon={PROVIDER_ICON[p.id]}
        size={13}
        strokeWidth={1.75}
...
function ProviderConfigureCTA({ providerId }: { providerId: ProviderId }) {
  ...
  return (
    <button
      ...
      className="group mx-2 mb-1 flex w-[calc(100%-1rem)] items-center gap-2 rounded-md border border-dashed border-border/70 bg-muted/20 px-3 py-2 text-left text-[13px] text-muted-foreground transition-colors hover:border-border hover:bg-accent/40 hover:text-foreground"
    >
