...
import { SLASH_COMMANDS, GENCODE_CMD_RE } from "../lib/slashCommands";
...
function CommandSnippet({ name }: { name: string }) {
  ...
  if (!meta) {
    return (
      <div className="inline-flex items-center gap-1.5 rounded-md border border-border/50 bg-muted/40 px-2 py-1 font-mono text-[13px]">
        /{name}
      </div>
    );
  }
  ...
  return (
    <div className="mb-1 flex flex-wrap gap-1">
      {chips.map((c, i) => (
        <span
          key={i}
          className="inline-flex items-center gap-1 rounded-md border border-border/50 bg-card/60 px-1.5 py-0.5 text-[13px] text-muted-foreground"
        >
          {chipIcon(c)}
...
const CompactionNotice = memo(function CompactionNotice({
  ...
  return (
    <div className="flex items-center gap-2 rounded-md border border-border/40 bg-muted/30 px-2.5 py-1.5 text-[13px] text-muted-foreground">
      <span className="size-1.5 shrink-0 rounded-full bg-amber-500/80" />
      <span className="flex-1 truncate">
        上下文已压缩 — 已省略 {droppedCount} 条较旧的工具结果以节省 Token。
      </span>
      <button
        type="button"
        onClick={onDismiss}
...
const RenderedMessage = memo(function RenderedMessage({
  ...
  if (message.role === "user") {
    ...
    const cmdMatch = rawText.match(GENCODE_CMD_RE);
  ...
  return (
    <Collapsible className="group/read overflow-hidden rounded-md border border-border/50 bg-card/50">
      ...
      >
        ...
        {paths.length > 0 ? (
          <span className="min-w-0 flex-1 truncate font-mono text-[13px] text-muted-foreground/80 group-data-[state=open]/read:invisible">
            · {preview}
          </span>
        ) : null}
      </CollapsibleTrigger>
      <CollapsibleContent className="gencode-collapsible-content border-t border-border/30">
        <ul className="flex flex-col gap-0.5 px-2 py-1.5">
          {paths.map((path) => (
            <li
              key={path}
              className="flex items-center gap-1.5 font-mono text-[13px] text-muted-foreground"
            >
              <HugeiconsIcon
                icon={File01Icon}
                size={10}
