...
const TOOL_META: Record<string, { label: string; icon: typeof FilePlusIcon }> =
  {
    ...
    multi_edit: { label: "批量编辑文件", icon: Edit02Icon },
    create_directory: { label: "创建目录", icon: FolderAddIcon },
    bash_run: { label: "执行 Shell 命令", icon: TerminalIcon },
...
function PreviewBlock({
  ...
  if (toolName === "bash_run" || toolName === "bash_background") {
    ...
    return (
      <div className="space-y-1.5">
        ...
        <pre
          className={cn(
            "max-h-40 overflow-auto rounded-md bg-muted/60 p-2 font-mono text-[13px] leading-relaxed",
          )}
        >
  ...
  return (
    <pre className="overflow-auto rounded-md bg-muted/60 p-2 font-mono text-[13px] leading-relaxed">
