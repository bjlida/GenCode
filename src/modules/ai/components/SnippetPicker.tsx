...
export function SnippetPickerContent({
  ...
  const aiCommands = commands.filter((it) => it.kind === "command" && it.command.source === "ai");
  const ccCommands = commands.filter((it) => it.kind === "command" && it.command.source === "cc");
    ...
    >
      {items.length === 0 ? (
        ...
        <div className="max-h-72 overflow-y-auto py-1">
            ...
            <>
              ...
              <ul>
                {aiCommands.map((it) => {
                  ...
                  const c = it.command;
                  return renderCommand(c, i, lead, activeIndex, onPick, onHover);
                })}
              </ul>
            </>
          )}
          {ccCommands.length > 0 && (
            <>
              <SectionHeader label="Claude Code 命令" />
              <ul>
                {ccCommands.map((it) => {
                  cursor += 1;
                  const i = cursor;
...
function SectionHeader({ label }: { label: string }) {
  return (
    <div className="px-2 pt-1.5 pb-1 text-[13px] font-medium uppercase tracking-wide text-muted-foreground/70">
      {label}
    </div>
  );
}
