...
export function FilePickerContent({
  ...
  return (
    ...
    >
      <div className="border-b border-border/60 px-2.5 py-1.5 text-[13px] font-medium uppercase tracking-wide text-muted-foreground/70">
        工作区文件
      </div>
      {!hasWorkspace ? (
        ...
        <>
          ...
          {truncated && (
            <div className="border-t border-border/60 px-2.5 py-1.5 text-[13px] text-muted-foreground">
              工作区较大 — 请细化搜索条件以缩小结果范围。
            </div>
          )}
        </>
      )}
