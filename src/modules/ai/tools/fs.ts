...
import { checkSandboxRead, checkSandboxWrite } from "../lib/sandbox";
import { newQueuedEditId, usePlanStore } from "../store/planStore";
import { resolvePath, type ToolContext } from "./context";
import { djb2 } from "@/lib/utils";
  ...
  return {
    read_file: tool({
      ...
      execute: async ({ path, offset, limit }) => {
        ...
        const sandbox = await checkSandboxRead(safety.canonical);
        if (!sandbox.allowed) return { error: sandbox.reason ?? "沙箱策略拒绝", path: safety.canonical };
        const abs = safety.canonical;
        try {
          const r = await native.readFile(abs);
          if (r.kind === "binary")
            return { error: "拒绝二进制文件", path: abs, size: r.size };
          if (r.kind === "toolarge")
            return {
          ...
          if (isFullRead) {
            ...
            return {
              ...
              ...(truncated
                ? { truncated: true, hint: "请使用 offset 参数继续读取" }
                : {}),
            };
          }
    ...
    write_file: tool({
      ...
      execute: async ({ path, content }) => {
        ...
        if (!safety.ok) return { error: safety.reason, path: reqPath };
        const sandbox = await checkSandboxWrite(safety.canonical);
        if (!sandbox.allowed) return { error: sandbox.reason ?? "沙箱策略拒绝写入", path: safety.canonical };
        const abs = safety.canonical;
