...
export const DEFAULT_THEME_ID = "gencode-default";
...
export type Preferences = {
  ...
  aiMaxTokens: number;
  permFileWrite: boolean;
  permTerminalExec: boolean;
  permProjectScan: boolean;
...
const KEY_QWEN_BASE_URL = "qwenBaseURL";
const KEY_QWEN_MODEL_ID = "qwenModelId";
const KEY_ZHIPU_BASE_URL = "zhipuBaseURL";
const KEY_ZHIPU_MODEL_ID = "zhipuModelId";
const KEY_MOONSHOT_BASE_URL = "moonshotBaseURL";
const KEY_MOONSHOT_MODEL_ID = "moonshotModelId";
const KEY_AI_TEMPERATURE = "aiTemperature";
const KEY_AI_MAX_TOKENS = "aiMaxTokens";
const KEY_PERM_FILE_WRITE = "permFileWrite";
const KEY_PERM_TERMINAL_EXEC = "permTerminalExec";
const KEY_PERM_PROJECT_SCAN = "permProjectScan";
...
export const DEFAULT_PREFERENCES: Preferences = {
  ...
  openrouterModelId: "",
  qwenBaseURL: QWEN_DEFAULT_BASE_URL,
  qwenModelId: "",
  ...
  aiMaxTokens: 8192,
  permFileWrite: true,
  permTerminalExec: true,
  permProjectScan: true,
  favoriteModelIds: [],
  recentModelIds: [],
  vimMode: false,
...
// window can listen.
const PREFS_CHANGED_EVENT = "gencode://prefs-changed";
...
export async function loadPreferences(): Promise<Preferences> {
  ...
  return {
    ...
    qwenBaseURL:
      get<string>(KEY_QWEN_BASE_URL) ?? DEFAULT_PREFERENCES.qwenBaseURL,
    qwenModelId:
      get<string>(KEY_QWEN_MODEL_ID) ?? DEFAULT_PREFERENCES.qwenModelId,
    zhipuBaseURL:
      get<string>(KEY_ZHIPU_BASE_URL) ?? DEFAULT_PREFERENCES.zhipuBaseURL,
    zhipuModelId:
      get<string>(KEY_ZHIPU_MODEL_ID) ?? DEFAULT_PREFERENCES.zhipuModelId,
    moonshotBaseURL:
      get<string>(KEY_MOONSHOT_BASE_URL) ?? DEFAULT_PREFERENCES.moonshotBaseURL,
    moonshotModelId:
      get<string>(KEY_MOONSHOT_MODEL_ID) ?? DEFAULT_PREFERENCES.moonshotModelId,
    aiTemperature:
      get<number>(KEY_AI_TEMPERATURE) ?? DEFAULT_PREFERENCES.aiTemperature,
    aiMaxTokens:
      get<number>(KEY_AI_MAX_TOKENS) ?? DEFAULT_PREFERENCES.aiMaxTokens,
    permFileWrite:
      get<boolean>(KEY_PERM_FILE_WRITE) ?? DEFAULT_PREFERENCES.permFileWrite,
    permTerminalExec:
      get<boolean>(KEY_PERM_TERMINAL_EXEC) ?? DEFAULT_PREFERENCES.permTerminalExec,
    permProjectScan:
      get<boolean>(KEY_PERM_PROJECT_SCAN) ?? DEFAULT_PREFERENCES.permProjectScan,
    favoriteModelIds: (
...
export async function setQwenBaseURL(value: string): Promise<void> {
  await writePref(KEY_QWEN_BASE_URL, value);
}

export async function setQwenModelId(value: string): Promise<void> {
  await writePref(KEY_QWEN_MODEL_ID, value);
}
...
export async function setZhipuModelId(value: string): Promise<void> {
  await writePref(KEY_ZHIPU_MODEL_ID, value);
}

export async function setMoonshotBaseURL(value: string): Promise<void> {
  await writePref(KEY_MOONSHOT_BASE_URL, value);
}

export async function setMoonshotModelId(value: string): Promise<void> {
  await writePref(KEY_MOONSHOT_MODEL_ID, value);
}

export async function setAiTemperature(value: number): Promise<void> {
  const clamped = Number.isFinite(value)
    ? Math.min(2, Math.max(0, Math.round(value * 10) / 10))
    : DEFAULT_PREFERENCES.aiTemperature;
  await writePref(KEY_AI_TEMPERATURE, clamped);
}

export async function setAiMaxTokens(value: number): Promise<void> {
  const clamped = Number.isFinite(value)
    ? Math.min(65536, Math.max(256, Math.round(value)))
    : DEFAULT_PREFERENCES.aiMaxTokens;
  await writePref(KEY_AI_MAX_TOKENS, clamped);
}

export async function setPermFileWrite(value: boolean): Promise<void> {
  await writePref(KEY_PERM_FILE_WRITE, value);
}

export async function setPermTerminalExec(value: boolean): Promise<void> {
  await writePref(KEY_PERM_TERMINAL_EXEC, value);
}

export async function setPermProjectScan(value: boolean): Promise<void> {
  await writePref(KEY_PERM_PROJECT_SCAN, value);
}
...
export async function onPreferencesChange(
  ...
  const map: Record<string, PrefKey> = {
    ...
    [KEY_QWEN_BASE_URL]: "qwenBaseURL",
    [KEY_QWEN_MODEL_ID]: "qwenModelId",
    [KEY_ZHIPU_BASE_URL]: "zhipuBaseURL",
    [KEY_ZHIPU_MODEL_ID]: "zhipuModelId",
    [KEY_MOONSHOT_BASE_URL]: "moonshotBaseURL",
    [KEY_MOONSHOT_MODEL_ID]: "moonshotModelId",
    [KEY_AI_TEMPERATURE]: "aiTemperature",
    [KEY_AI_MAX_TOKENS]: "aiMaxTokens",
    [KEY_PERM_FILE_WRITE]: "permFileWrite",
    [KEY_PERM_TERMINAL_EXEC]: "permTerminalExec",
    [KEY_PERM_PROJECT_SCAN]: "permProjectScan",
  ...
  // to every webview including the writer. Listening only to the event avoids
  // same-window double-callbacks from LazyStore.onChange + event delivery.
...
const KEYS_CHANGED_EVENT = "gencode://ai-keys-changed";
