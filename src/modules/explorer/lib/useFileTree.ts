...
export function useFileTree(rootPath: string | null, options?: Options) {
  ...
  const fetchChildren = useCallback(async (path: string) => {
    ...
    try {
      ...
      setNodes((s) => {
        const existing = s[path];
        if (existing?.status === "loaded" && !dead.size) {
          const old = existing.entries;
          if (
  ...
  // Throttle fs events: batch changes with a short debounce, but never refresh
  // more often than MIN_INTERVAL.  This prevents the sidebar from strobing when
  // background processes (indexers, LSP, git, antivirus) generate continuous
  // filesystem noise.
  const fsBatchRef = useRef<Set<string>>(new Set());
  const fsTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const lastRefreshRef = useRef(0);
  const pendingRefreshRef = useRef(false);
  const FS_DEBOUNCE_MS = 400;
  const FS_MIN_INTERVAL_MS = 5000;

  function flushFsBatch(alive: boolean) {
    if (!alive) return;
    const dirs = new Set(fsBatchRef.current);
    fsBatchRef.current.clear();
    ...
    if (elapsed >= FS_MIN_INTERVAL_MS) {
      lastRefreshRef.current = now;
      for (const d of dirs) void fetchChildren(d);
    } else {
      // Within cooldown — schedule a deferred refresh
      if (pendingRefreshRef.current) return; // already waiting
      pendingRefreshRef.current = true;
      fsTimerRef.current = setTimeout(() => {
        fsTimerRef.current = null;
        // Re-check: batch may have grown while we waited
        if (fsBatchRef.current.size > 0 || dirs.size > 0) {
          const all = new Set([...fsBatchRef.current, ...dirs]);
          fsBatchRef.current.clear();
          lastRefreshRef.current = Date.now();
          pendingRefreshRef.current = false;
          for (const d of all) void fetchChildren(d);
        }
      }, FS_MIN_INTERVAL_MS - elapsed);
    }
  }
  ...
  useEffect(() => {
    ...
    void listenFsChanged((paths) => {
      ...
      const batch = fsBatchRef.current;
      for (const p of paths) {
        const parent = dirname(p);
        if (current[parent]?.status === "loaded") batch.add(parent);
        if (current[p]?.status === "loaded") batch.add(p);
      }
      if (batch.size === 0) return;

      // No active debounce timer → start one
      if (!fsTimerRef.current || pendingRefreshRef.current) {
        if (fsTimerRef.current) clearTimeout(fsTimerRef.current);
        fsTimerRef.current = setTimeout(() => flushFsBatch(alive), FS_DEBOUNCE_MS);
      }
    }).then((un) => {
