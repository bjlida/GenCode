...
export function ModelsSection() {
  ...
  return (
    <div className="flex flex-col gap-7">
      <SectionHeader
        title="模型"
        description="连接你使用的服务商。密钥存储在操作系统密钥链中，仅由灵码ADE 使用。"
      />
      ...
      <div className="flex flex-col gap-3">
        ...
        {visibleProviders.length === 0 ? (
          <div className="rounded-lg border border-dashed border-border/30 bg-card/40 px-4 py-8 text-center">
            <p className="text-[12px] text-muted-foreground">
              暂无已连接的服务商。
            </p>
            <p className="mt-0.5 text-[10.5px] text-muted-foreground/70">
              点击"添加服务商"连接云端或本地模型来源。
            </p>
          </div>
        ) : (
...
function DefaultsBlock({
  ...
  return (
    <div className="flex flex-col gap-3">
      <Label>默认设置</Label>
      <div className="flex flex-col gap-2.5 rounded-lg border border-border/50 bg-background px-3 py-2">
        <FieldRow label="对话模型">
          <DefaultModelPicker
            defaultModel={defaultModel}
            configuredIds={configuredIds}
...
function LocalProviderCard({
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-border/50 bg-background px-3 py-2">
      <div className="flex items-center gap-2">
        ...
        {configured ? (
          <Badge
            variant="outline"
            className="ml-1 h-4 gap-1 border-border/30 bg-muted/40 px-1.5 text-[10px] font-normal text-muted-foreground"
          >
