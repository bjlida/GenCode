  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        {SHORTCUT_GROUPS.map((group) => {
          ...
          return (
            <div key={group} className="flex flex-col gap-3">
              ...
              <div className="flex flex-col divide-y divide-border/40 rounded-lg border border-border/30 bg-card/40 overflow-hidden">
                {items.map((s) => (
                  <ShortcutRow
                    key={s.id}
                    shortcut={s}
      ...
      <AlertDialog open={resetDialogOpen} onOpenChange={setResetDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>恢复所有默认快捷键？</AlertDialogTitle>
            <AlertDialogDescription>
              此操作将恢复所有自定义键盘快捷键为出厂默认值，无法撤消。
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function Recorder({
  ...
  return (
    <div className="flex items-center gap-2 rounded bg-accent/50 px-2 py-1 text-[11px] ring-1 ring-accent">
      ...
      <span className="text-muted-foreground">(Esc 取消)</span>
    </div>
  );
}
