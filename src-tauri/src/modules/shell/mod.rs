...
pub async fn shell_run_command(
    ...
    sandbox_state: tauri::State<'_, sandbox::SandboxState>,
    ...
    // Server-side sandbox enforcement — reject denied commands regardless
    // of what the frontend reports.
    let policy = sandbox_state.policy.lock().map_err(|e| e.to_string())?;
    if let EnforcementResult::Denied(reason) = check_command(&policy, &trimmed) {
        return Err(format!("命令被安全策略拒绝: {reason}"));
    }
    drop(policy);
...
pub async fn shell_session_run(
    ...
    let trimmed = command.trim().to_string();
    // Server-side sandbox enforcement.
    let policy = sandbox_state.policy.lock().map_err(|e| e.to_string())?;
    if let EnforcementResult::Denied(reason) = check_command(&policy, &trimmed) {
        return Err(format!("命令被安全策略拒绝: {reason}"));
    }
...
pub fn shell_bg_spawn(
    state: tauri::State<ShellState>,
    registry: tauri::State<WorkspaceRegistry>,
    sandbox_state: tauri::State<sandbox::SandboxState>,
    command: String,
    cwd: Option<String>,
    workspace: Option<WorkspaceEnv>,
) -> Result<u32, String> {
