...
fn generate_sentinel() -> String {
    ...
    format!("__GENCODE_CWD_{:016x}_{:016x}__", mix, counter)
}
...
fn wrap_with_sentinel(command: &str, workspace: &WorkspaceEnv, sentinel: &str) -> String {
    ...
    #[cfg(windows)]
    {
        // Detect the configured native shell so we emit the right sentinel syntax.
        // build_oneshot_command makes the same detection and picks /C (cmd) or
        // -Command (pwsh), so the sentinel must match.
        ...
        let is_cmd = shell
            ...
            .unwrap_or(false);
        if is_cmd {
            // cmd.exe: the sentinel line is appended to stdout. %CD% reports the
            // working directory (which session commands don't change, but the
            // marker still serves as an end-of-output sentinel).
            format!("{command}\necho {sentinel}%CD%\n")
        } else {
            format!(
                "{command}\n$__gencode_rc = if ($null -ne $LASTEXITCODE) {{ $LASTEXITCODE }} elseif ($?) {{ 0 }} else {{ 1 }}\n\"`n{sentinel}$($PWD.Path)\"\nexit $__gencode_rc\n",
            )
        }
    }
}
...
mod tests {
    ...
    fn sentinels_are_unique_per_session() {
        ...
        assert!(a.sentinel.starts_with("__GENCODE_CWD_"));
    ...
    fn strip_uses_session_sentinel_only() {
        ...
        let attacker = "__GENCODE_CWD_0000000000000000_0000000000000000__/evil";
        let trailer = format!("\n{}/real\n", s.sentinel);
    ...
    fn strip_returns_none_when_session_sentinel_absent() {
        let s = ShellSession::new("/tmp".into(), WorkspaceEnv::Local);
        let stdout = "some output\n__GENCODE_CWD_aaaa_bbbb__/spoof\nmore\n";
