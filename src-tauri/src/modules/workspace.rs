...
pub fn bootstrap_registry(registry: &WorkspaceRegistry) {
    let launch_dir = resolve_launch_dir();
    let _ = registry.authorize(&launch_dir);
    // NOTE: $HOME is NOT automatically authorized. The sandbox policy's
    // denied_paths list protects sensitive files (.ssh/, .env, *.pem, etc.)
    // at the backend level (via require_authorized). Users can authorize
    // additional directories through the workspace UI.
}
...
pub fn wsl_path_to_unc(distro: &str, path: &str) -> PathBuf {
    ...
    if !is_safe_distro_name(distro) {
        return PathBuf::from(r"\\wsl.localhost\__gencode_invalid_distro__");
    }
    let normalized = path.replace('\\', "/");
...
mod tests {
    ...
    fn wsl_path_to_unc_blocks_traversal_distro() {
        ...
        assert!(s.contains("__gencode_invalid_distro__"), "got: {s}");
...
mod auth_tests {
    ...
    fn authorize_user_spawn_cwd_rejects_missing_path() {
        let mut missing = env::temp_dir();
        missing.push(format!("gencode-user-missing-{}", std::process::id()));
        let reg = WorkspaceRegistry::default();
        let s = missing.to_string_lossy().into_owned();
    ...
    fn resolve_launch_cwd_ignores_nonexistent_cli_dir() {
        ...
        let resolved = resolve_launch_cwd(Some("/no/such/gencode/dir"), Some(env.clone()));
