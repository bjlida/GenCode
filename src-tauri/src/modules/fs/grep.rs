...
pub fn fs_grep(
    ...
    let root_path = super::require_authorized(&registry, &workspace, &root)?;
    if !root_path.is_dir() {
    ...
    walker.run(|| {
        ...
        Box::new(move |dent_res| {
            ...
            let path = dent.path();
            // `ignore` does not descend into symlink dirs with follow_links(false),
            // but file symlinks can still appear and `metadata`/search_path would
            // follow them. Refuse them so grep cannot read outside the workspace.
