...
const OWNED_MARKERS: [&str; 2] = ["notify;GenCode;", "gencode;notify"];

// Gated on GENCODE_TERMINAL; no-op outside GenCode. Returns the sequence via
// `terminalSequence` because hooks lost /dev/tty access in v2.1.139.
fn hook_cmd(event: &str) -> String {
    format!(
        r#"[ -n "$GENCODE_TERMINAL" ] && printf '{{"terminalSequence":"\\u001b]777;notify;GenCode;{event}\\u0007"}}' || true"#
    )
}
...
pub fn agent_claude_hooks_status() -> bool {
    ...
    HOOK_EVENTS
        .iter()
        .all(|(_, m)| content.contains(&format!("notify;GenCode;{m}")))
}
...
mod tests {
    ...
    fn adds_all_event_hooks_to_empty_config() {
        ...
        assert!(command(&out, "Notification", 0).contains("notify;GenCode;attention"));
        assert!(command(&out, "Stop", 0).contains("notify;GenCode;finished"));
        assert!(command(&out, "UserPromptSubmit", 0).contains("notify;GenCode;working"));
        ...
        let legacy = json!({
            "hooks": {
                "Notification": [
                    { "hooks": [ {
                        "type": "command",
                        "command": "[ -n \"$GENCODE_TERMINAL\" ] && printf '\\033]777;gencode;notify\\033\\\\' > /dev/tty || true"
                    } ] }
                ]
            }
        });
