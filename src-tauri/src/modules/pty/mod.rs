...
/// Open a PTY session with a pre-built Command (bypasses shell init).
/// Used by claude_code to spawn Node.js + Claude Code directly.
pub async fn pty_open_command(
    app: tauri::AppHandle,
    state: tauri::State<'_, PtyState>,
    cols: u16,
    rows: u16,
    ...
    .await
    .map_err(|e| {
        log::error!("pty_open_command join failed: {e}");
        e.to_string()
    })?
    .map_err(|e| {
        log::error!("pty_open_command failed: {e}");
        e
    })?;
    state.sessions.write().unwrap().insert(id, session);
    log::info!("pty (command) opened id={id} cols={cols} rows={rows}");
    Ok(id)
}
...
pub fn pty_close(state: tauri::State<PtyState>, id: u32) -> Result<(), String> {
    ...
    if let Some(s) = session {
        ...
        thread::Builder::new()
            .name(format!("gencode-pty-drop-{id}"))
            .spawn(move || {
