...
mod unix {
    ...
    fn prepare_fish_conf_d() -> Result<(), String> {
        ...
        fs::create_dir_all(&dir).map_err(|e| format!("create {}: {e}", dir.display()))?;
        write_if_changed(&dir.join("gencode.fish"), FISH_INIT)?;
        Ok(())
    }
...
mod windows {
        ...
        match (shell_kind, integration) {
            (
                ...
                if let Some(user_zdotdir) = user_zdotdir {
                    args.push(format!("GENCODE_USER_ZDOTDIR={user_zdotdir}"));
                }
    ...
    fn prepare_wsl_integration_dir(distro: &str, shell: &str) -> Result<(String, PathBuf), String> {
        ...
        let linux_dir = format!(
            "{}/.cache/gencode/shell-integration/{shell}",
    ...
    fn prepare_wsl_fish_conf_d(distro: &str) -> Result<(), String> {
        ...
        let unc_dir = crate::modules::workspace::wsl_path_to_unc(distro, &linux_dir);
        fs::create_dir_all(&unc_dir).map_err(|e| format!("create {}: {e}", unc_dir.display()))?;
        let unc_file = unc_dir.join("gencode.fish");
    ...
    fn prepare_ps_profile() -> Result<PathBuf, String> {
        ...
        // PowerShell 5.1 reads .ps1 files using the system ANSI codepage (GBK on
        // Chinese Windows). Without a UTF-8 BOM, Chinese characters are garbled.
        write_utf8_with_bom(&file, PROFILE_PS1)?;
        Ok(file)
    }
    ...
    fn write_utf8_with_bom(path: &Path, content: &str) -> Result<(), String> {
        // Only skip if the file already starts with a BOM AND has matching content.
        // Without this check we'd never rewrite a file that was previously written
        // without BOM (e.g. by an older version of write_if_changed).
        if let Ok(existing) = fs::read(path) {
            if existing.starts_with(b"\xEF\xBB\xBF") && &existing[3..] == content.as_bytes() {
                return Ok(());
            }
        }
        let mut tmp: OsString = path.as_os_str().to_owned();
        tmp.push(".__gencode_tmp__");
        let tmp = PathBuf::from(tmp);
        let mut bom = Vec::with_capacity(3 + content.len());
        bom.extend_from_slice(b"\xEF\xBB\xBF");
        bom.extend_from_slice(content.as_bytes());
        fs::write(&tmp, &bom).map_err(|e| format!("write {}: {e}", tmp.display()))?;
        fs::rename(&tmp, path).map_err(|e| {
            let _ = fs::remove_file(&tmp);
            format!("rename {} -> {}: {e}", tmp.display(), path.display())
        })
    ...
    mod tests {
        ...
        fn builds_wsl_zsh_launch_spec_with_env_and_login() {
            let spec = build_wsl_launch_spec(
                ...
                ShellKind::Zsh,
                WslShellIntegration::Zsh {
                    zdotdir: "/home/vinicios/.cache/gencode/shell-integration/zsh".into(),
                    user_zdotdir: None,
                },
            );
            assert_eq!(
                ...
                vec![
                    ...
                    "--exec".to_string(),
                    "env".to_string(),
                    "ZDOTDIR=/home/vinicios/.cache/gencode/shell-integration/zsh".to_string(),
        ...
        fn builds_wsl_zsh_launch_spec_with_user_zdotdir_probe() {
            ...
            assert_eq!(
                ...
                vec![
                    ...
                    "--exec".to_string(),
                    "env".to_string(),
                    "GENCODE_USER_ZDOTDIR=/home/vinicios/.config/zsh".to_string(),
                    "ZDOTDIR=/home/vinicios/.cache/gencode/shell-integration/zsh".to_string(),
                    "/usr/bin/zsh".to_string(),
                    "-l".to_string(),
                ]
        ...
        fn builds_wsl_bash_launch_spec_with_rcfile() {
            let spec = build_wsl_launch_spec(
                ...
                WslShellIntegration::Bash {
                    rcfile: "/home/vinicios/.cache/gencode/shell-integration/bash/bashrc".into(),
                },
            );
            assert_eq!(
                spec.args,
                vec![
                    ...
                    "/bin/bash".to_string(),
                    "--rcfile".to_string(),
                    "/home/vinicios/.cache/gencode/shell-integration/bash/bashrc".to_string(),
                    "-i".to_string(),
                ]
            );
        }
