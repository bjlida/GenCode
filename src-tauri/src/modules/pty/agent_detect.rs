...
const GENCODE_MARKER: &[u8] = b"notify;GenCode;";
...
impl AgentDetector {
    ...
    fn handle_osc777<F: FnMut(Transition)>(&mut self, pt: &[u8], emit: &mut F) {
        if let Some(event) = pt.strip_prefix(GENCODE_MARKER) {
...
mod tests {
    ...
    #[test]
    fn gencode_marker_drives_status() {
        let mut d = AgentDetector::new();
        run(&mut d, &osc("133;C;claude"));
        assert_eq!(run(&mut d, &osc("777;notify;GenCode;attention")), vec![Transition::Attention]);
        assert_eq!(run(&mut d, &osc("777;notify;GenCode;working")), vec![Transition::Working]);
        assert!(run(&mut d, &osc("777;notify;GenCode;working")).is_empty());
        assert_eq!(run(&mut d, &osc("777;notify;GenCode;finished")), vec![Transition::Finished]);
    }

    #[test]
    fn gencode_marker_auto_arms_without_preexec() {
        let mut d = AgentDetector::new();
        assert_eq!(
            run(&mut d, &osc("777;notify;GenCode;attention")),
