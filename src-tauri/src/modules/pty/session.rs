...
const AGENT_EVENT: &str = "gencode:agent-signal";
...
const OVERFLOW_NOTICE: &[u8] =
    b"\x1bc\x1b[2m[gencode: dropped output due to backpressure]\x1b[0m\r\n";
...
/// Spawn a PTY session with a pre-built `std::process::Command`.
/// Bypasses shell initialization — used for Claude Code and other
/// non-shell child processes.
    ...
    let cmd: portable_pty::CommandBuilder = {
        ...
        for (k, v) in command.get_envs() {
            if let Some(v) = v {
                ...
                // Remove the env var (set to None).
                // CommandBuilder doesn't have an unset method, skip.
                let _ = (k, v);
            }
        }
