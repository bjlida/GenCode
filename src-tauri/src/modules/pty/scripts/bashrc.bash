# gencode-shell-integration (bashrc)
#
...
if [ -z "$__GENCODE_HOOKS_LOADED" ]; then
  ...
  _gencode_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_gencode_urlencode "$PWD")"
    if [ -z "$__GENCODE_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __GENCODE_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_gencode_precmd:"*) ;;
    *) PROMPT_COMMAND="_gencode_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac
