...
**GenCode / 灵码ADE** 是一款 AI 原生终端与轻量开发工作空间。本文档对应 **v1.0.0** 产品功能说明。
...
| **右侧** | AI 助手面板（`Ctrl+I`，宽度 380–680px 可拖拽并记忆，默认约 460px） |
...
- 若布局变化后终端只剩光标，请 **新建终端标签**（已修复 WebGL 重绘）
...
安装后在普通终端标签中执行 `claude`。每次通过状态栏 CC 图标或 AI 工具启动 claude 时，GenCode 会**提前重新合并**中文 locale（`language: chinese`、`zh_CN.UTF-8`）到 `~/.claude/settings.json`，防止外部代理管理工具覆盖后失效。

> **说明**：`language: chinese` 控制 **claude AI 回复语言**，不影响 CLI 界面文字（CLI 无内置 i18n）。
...
- Vim 模式、自动换行、状态栏行列号
- 关闭未保存文件：保存 / 不保存 / 取消
- **11 种内置主题**；**GenCode Dark** 为暗色默认（终端配色风格：红色关键字、黄色字符串、绿色函数/属性、蓝色数字、灰色注释）
...
- **编辑器**: 修复深色主题下代码内容不可见（主题模式与编辑器配色极性不匹配）；新增 **GenCode Dark** 编辑器主题（暗色默认）
- **Claude Code**: 每次启动 claude 前重新合并中文 locale，防止外部 API 代理工具覆盖 `~/.claude/settings.json` 后中文失效
